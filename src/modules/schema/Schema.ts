...
class Schema {
  ...
  private generateReadQueries() {
    this._readAllQuery = gql`${get_all(this.tableName, this.schemaKeys)}`
    this._readByPKQuery = gql`${get_by_pk(this.tableName, this.schemaKeys, this._primaryKey)}`
  }

  private generateCreateQueries() {
    this._createOneQuery = gql`${create_one(this.tableName, this.schemaKeys, this._primaryKey)}`
